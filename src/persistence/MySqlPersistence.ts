...
export class MySqlPersistence<T> implements IReferenceable, IUnreferenceable, IConfigurable, IOpenable, ICleanable {
    ...
    public open(correlationId: string, callback?: (err: any) => void): void {
        ...
        let openCurl = (err) => {
            if (err == null && this._connection == null) {
                err = new InvalidStateException(correlationId, 'NO_CONNECTION', 'MySQL connection is missing');
            }

            if (err == null && !this._connection.isOpen()) {
                err = new ConnectionException(correlationId, "CONNECT_FAILED", "MySQL connection is not opened");
            }
    ...
    public close(correlationId: string, callback?: (err: any) => void): void {
        ...
        if (this._connection == null) {
            callback(new InvalidStateException(correlationId, 'NO_CONNECTION', 'MySql connection is missing'));
            return;
        }
    ...
    public clear(correlationId: string, callback?: (err: any) => void): void {
        ...
        this._client.query(query, (err, result) => {
            if (err) {
                err = new ConnectionException(correlationId, "CONNECT_FAILED", "Connection to mysql failed")
                    .withCause(err);
            }
