...
class MySqlPersistence {
    ...
    open(correlationId, callback) {
        ...
        let openCurl = (err) => {
            if (err == null && this._connection == null) {
                err = new pip_services3_commons_node_5.InvalidStateException(correlationId, 'NO_CONNECTION', 'MySQL connection is missing');
            }
            if (err == null && !this._connection.isOpen()) {
                err = new pip_services3_commons_node_4.ConnectionException(correlationId, "CONNECT_FAILED", "MySQL connection is not opened");
            }
            this._opened = false;
            if (err) {
                if (callback)
    ...
    close(correlationId, callback) {
        ...
        if (this._connection == null) {
            callback(new pip_services3_commons_node_5.InvalidStateException(correlationId, 'NO_CONNECTION', 'MySql connection is missing'));
            return;
        }
        let closeCurl = (err) => {
            this._opened = false;
    ...
    clear(correlationId, callback) {
        ...
        this._client.query(query, (err, result) => {
            if (err) {
                err = new pip_services3_commons_node_4.ConnectionException(correlationId, "CONNECT_FAILED", "Connection to mysql failed")
                    .withCause(err);
            }
            if (callback)
                callback(err);
...
MySqlPersistence._defaultConfig = pip_services3_commons_node_1.ConfigParams.fromTuples("collection", null, "dependencies.connection", "*:connection:mysql:*:1.0", 
// connections.*
// credential.*
